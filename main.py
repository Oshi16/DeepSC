...
if __name__ == '__main__':
    ...
    # Evaluate and plot reconstruction loss vs SNR
    snr_values = np.arange(0, 21, 2)  # SNR range from 0 to 20 dB with step of 2
    reconstruction_losses = []

    for snr in snr_values:
        rec_loss = validate(epoch, args, deepsc, criterion, snr=snr)
        reconstruction_losses.append(rec_loss)

    # Plot reconstruction loss vs SNR
    plt.figure(figsize=(10, 5))
    plt.plot(snr_values, reconstruction_losses, marker='o', linestyle='-', color='b', label='Reconstruction Loss')
    plt.xlabel('SNR (dB)')
